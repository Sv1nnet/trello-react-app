    ...
    try {
      const board = await Board.findById(boardId)
        .catch((err) => {
          ...
          return Promise.reject(new Error('Could not remove a label'));
        });
      ...
      if (isOwner || (isMember && !board.isReadOnly)) {
        ...
        cardToUpdate.removeLabel(labelId);

        const updatedBoard = await board.save().catch((err) => {
          console.log('Could not save board with a removed label', err);
          return Promise.reject(new Error('Could not save board with a removed label'));
        });
